...
def execute_manual_trade(paper: dict[str, Any], price: float, inputs: dict[str, Any]) -> tuple[str, str, list[str], dict[str, float]]:
    ...
    if action == "BUY_USDT":
    ...
    elif action in {"BUY_USDT_FORCE", "BUY_OVERRIDE"}:
        ...
        if executed_usdt >= MIN_MANUAL_TRADE_USDT:
            ...
            btc += executed_btc
            cost_basis += executed_usdt
            reasons.append(f"Manual override buy executed: {executed_usdt:.2f} USDT at {price:.2f} USD.")
            reasons.append("Operator override: bot exposure corridor was intentionally ignored for this paper trade.")
            final_action = "MANUAL_BUY_OVERRIDE"
            confidence = "manual_override"
        else:
            executed_usdt = 0.0
            reasons.append("Manual override buy blocked: available cash/requested amount is below minimum useful trade size.")
            final_action = "MANUAL_BUY_OVERRIDE_BLOCKED"
            confidence = "manual_blocked"
        ...
        reasons.append(f"Manual action '{action}' is unknown. Allowed: BUY_USDT, BUY_OVERRIDE, SELL_PERCENT, SELL_ALL.")
