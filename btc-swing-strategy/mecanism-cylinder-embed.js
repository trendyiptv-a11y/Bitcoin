(function () {
  ...
  const T = {
    ro: { context:"CONTEXT", price:"PREȚ", participation:"PARTICIPARE", flow:"FLUX", liquidity:"LICHIDITATE", growth:"CONTEXT CREȘTERE", risk:"RISC STRUCTURAL", daily:"Interpretarea zilei", formula:"FORMULA ZILEI", full:"VEZI INTERPRETAREA COMPLETĂ", hide:"ASCUNDE INTERPRETAREA COMPLETĂ", waiting:"așteptăm date", signal:"SEMNAL", asset:"BTC", legendTitle:"LEGENDĂ SEMNALE", legendSubtitle:"Semnal auditat din mecanism; fallback text doar când câmpul lipsește.", footer:"Interpretare structurală experimentală, nu recomandare financiară." },
    en: { context:"CONTEXT", price:"PRICE", participation:"PARTICIPATION", flow:"FLOW", liquidity:"LIQUIDITY", growth:"GROWTH CONTEXT", risk:"STRUCTURAL RISK", daily:"Daily interpretation", formula:"DAILY FORMULA", full:"SEE FULL INTERPRETATION", hide:"HIDE FULL INTERPRETATION", waiting:"waiting data", signal:"SIGNAL", asset:"BTC", legendTitle:"SIGNAL LEGEND", legendSubtitle:"Audited mechanism signal; text fallback only when the field is missing.", footer:"Experimental structural interpretation, not financial advice." }
  };
  ...
  function pick(obj, roKey, enKey) { if (!obj) return ""; return lang() === "en" ? (obj[enKey] || obj[roKey] || "") : (obj[roKey] || obj[enKey] || ""); }
  function radarBlock(data) { if (!data) return {}; return lang() === "en" ? (data.radar_info_en || data.radar_info || {}) : (data.radar_info || data.radar_info_en || {}); }
  function traderCard(data) { return data?.trader_signal_card || data?.state?.trader_signal_card || data?.coeziv_state?.trader_signal_card || null; }
  ...
  function deriveFromText(text, type) {
    ...
    return contains(t, ["creștere confirmată", "growth confirmed"]) ? (en ? "growth" : "creștere") : (en ? "neutral" : "neutru");
  }
  ...
  function derive(data) {
    ...
    const all = [pick(data,"summary","summary_en"), pick(data,"plain_language","plain_language_en"), pick(data,"participation","participation_en"), pick(data,"risk_window","risk_window_en"), pick(data,"market_regime","market_regime_en"), pick(data,"watch_next","watch_next_en"), data?.message || ""].join(" | ");
    ...
    const visual = { growthPresent: card ? !!card.growth_confirmed : /prezent|present/i.test(growthText), riskReduced: reduced || !!r.visual?.risk_reduced, participationCohesive: /coeziv|cohesive/i.test(participationText), flowPositive: /pozitiv|positive/i.test(flowText), flowNegative: /negativ|negative/i.test(flowText) };
    const info = {
      date: fmtDate(data, r),
      ...
      formula: compact(r.formula || pick(data,"formula","formula_en") || pick(data,"plain_language","plain_language_en") || pick(data,"summary","summary_en") || data?.message, 185),
  ...
  function fallbackInfo() {
    const l = lang();
    return { date:"–", context:"–", price:T[l].waiting, participation:"–", flow:"–", liquidity:"–", growth:"–", risk:"–", formula:l === "en" ? "The card will be completed after the first approved daily analysis." : "Cardul va fi completat după prima analiză zilnică aprobată.", footer:T[l].footer, full:"", visual:{growthPresent:false,riskReduced:false,participationCohesive:false,flowPositive:false,flowNegative:false}, trader:{key:"no_data",audited:false} };
  }
  ...
  function css() {
    ...
    style.textContent = `
      ...
      .cxtr,.cxlg,.cxdc,.cxtr *,.cxlg *,.cxdc *{box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
      ...
      .cxdc{position:relative;border-radius:22px;border:1px solid rgba(34,238,255,.24);overflow:hidden;padding:12px;background:#031222;}.cxdc-core{position:relative;min-height:570px;border-radius:22px;border:1px solid rgba(34,238,255,.24);overflow:hidden;background:radial-gradient(circle at 74% 16%,rgba(0,245,255,.18),transparent 32%),linear-gradient(180deg,rgba(2,18,34,.86),rgba(0,10,22,.92));}.cxdc-labels{position:absolute;left:12px;top:72px;width:39%;max-width:160px;display:flex;flex-direction:column;gap:23px;z-index:5;}.cxdc-label{position:relative;min-height:43px;--c:#fff}.cxdc-label .k{font-size:clamp(15px,4.1vw,19px);font-weight:850;line-height:1.05;color:#fff}.cxdc-label .v{margin-top:4px;font-size:clamp(15px,4vw,19px);font-weight:760;line-height:1.08;color:var(--c)}.cxdc-label.context{--c:#bffcff}.cxdc-label.price{--c:#ffe600}.cxdc-label.part{--c:#ff9800}.cxdc-label.flow{--c:#14f4ff}.cxdc-label.liq{--c:#19a7ff}.cxdc-label.growth{--c:#ff3156}.cxdc-label.risk{--c:#ff6542}.cxdc-cylinder{position:absolute;right:8px;top:20px;width:57%;max-width:246px;height:520px;border-radius:40px 40px 24px 24px;border:1px solid rgba(175,245,255,.42);overflow:hidden;background:#062033;box-shadow:inset 0 0 42px rgba(0,240,255,.14),0 0 24px rgba(0,180,255,.10)}.cxdc-grid{position:absolute;inset:18px 10px;border-radius:26px;background:linear-gradient(90deg,rgba(200,240,255,.055) 1px,transparent 1px),linear-gradient(180deg,rgba(200,240,255,.045) 1px,transparent 1px);background-size:22px 22px;opacity:.38}.cxdc-beam{position:absolute;left:50%;top:34px;bottom:34px;width:8px;transform:translateX(-50%);background:linear-gradient(180deg,transparent,rgba(0,247,255,.55) 18%,rgba(170,250,255,.64) 50%,rgba(0,247,255,.55) 82%,transparent);box-shadow:0 0 18px rgba(0,240,255,.55);z-index:2}.cxdc-layer{position:absolute;left:17px;right:17px;height:46px;border-radius:50%;border:2px solid var(--c);box-shadow:0 0 16px var(--c);z-index:4}.cxdc-layer.context{top:66px;--c:#f8feff}.cxdc-layer.price{top:132px;--c:#ffe600}.cxdc-layer.part{top:198px;--c:#ff9800}.cxdc-layer.flow{top:264px;--c:#14f4ff}.cxdc-layer.liq{top:330px;--c:#19a7ff}.cxdc-layer.growth{top:396px;--c:#ff4968}.cxdc-risk{position:absolute;left:30px;right:30px;bottom:34px;height:54px;border-radius:50%;border:2px solid #ff4c25;background:rgba(255,75,32,.12);box-shadow:0 0 22px rgba(255,88,40,.58);z-index:5}.cxdc-off{position:absolute;left:50%;top:412px;transform:translateX(-50%);font-size:40px;color:#ff3156;text-shadow:0 0 12px #ff3156;z-index:7}
      ...
      @media(max-width:380px){.cxtr{padding:12px}.cxtr-icon{width:48px;height:48px;font-size:26px}.cxtr-grid{grid-template-columns:1fr}.cxdc-core{min-height:555px}.cxdc-labels{left:10px;width:40%;gap:20px}.cxdc-cylinder{right:5px;width:56%;height:505px}}
    `;
  ...
  function traderLegendHtml(activeKey) {
    const l = lang(), tt = T[l];
    const rows = ["buy","accumulate","wait","attention","sell","risk","no_data"].map((key) => {
      const meta = SIGNAL_META[key], tuple = l === "en" ? meta.en : meta.ro;
  ...
  function render(info) {
    ...
    if (!root) return;
    const l = lang(), tt = T[l], tr = info.trader || { key:"no_data", audited:false };
    const meta = SIGNAL_META[tr.key] || SIGNAL_META.wait;
    const tuple = l === "en" ? meta.en : meta.ro;
    const title = tr.title || tuple[0], subtitle = tr.subtitle || tuple[1], action = tr.action || tuple[2];
    const auditText = tr.audited ? "audit: trader_signal_card · backend" : "fallback: text-derived UI";
    root.innerHTML = `
      <div class="cxtr signal-${escHtml(meta.cls)}"><div class="cxtr-top"><div class="cxtr-asset">${tt.asset}</div><div>${escHtml(info.date)}</div></div><div class="cxtr-main"><div class="cxtr-icon">${escHtml(tr.icon || meta.icon)}</div><div><div class="cxtr-k">${tt.signal}</div><div class="cxtr-title">${escHtml(title)}</div></div></div><div class="cxtr-sub">${escHtml(subtitle)}</div><div class="cxtr-grid"><div class="cxtr-cell"><span>${tt.flow}</span><strong>${escHtml(tr.flow || info.flow || "–")}</strong></div><div class="cxtr-cell"><span>${tt.participation}</span><strong>${escHtml(tr.participation || info.participation || "–")}</strong></div><div class="cxtr-cell"><span>${tt.liquidity}</span><strong>${escHtml(tr.liquidity || info.liquidity || "–")}</strong></div><div class="cxtr-cell"><span>${tt.growth}</span><strong>${escHtml(tr.growth || info.growth || "–")}</strong></div></div><div class="cxtr-action">${escHtml(action)}</div><div class="cxtr-audit">${escHtml(auditText)}</div></div>
      ${traderLegendHtml(tr.key)}
      <div class="cxdc"><div class="cxdc-core"><div class="cxdc-labels"><div class="cxdc-label context"><div class="k">${T[l].context}</div><div class="v">${escHtml(info.context)}</div></div><div class="cxdc-label price"><div class="k">${T[l].price}</div><div class="v">${escHtml(info.price)}</div></div><div class="cxdc-label part"><div class="k">${T[l].participation}</div><div class="v">${escHtml(info.participation)}</div></div><div class="cxdc-label flow"><div class="k">${T[l].flow}</div><div class="v">${escHtml(info.flow)}</div></div><div class="cxdc-label liq"><div class="k">${T[l].liquidity}</div><div class="v">${escHtml(info.liquidity)}</div></div><div class="cxdc-label growth"><div class="k">${T[l].growth}</div><div class="v">${escHtml(info.growth)}</div></div><div class="cxdc-label risk"><div class="k">${T[l].risk}</div><div class="v">${escHtml(info.risk)}</div></div></div><div class="cxdc-cylinder"><div class="cxdc-grid"></div><div class="cxdc-beam"></div><div class="cxdc-layer context"></div><div class="cxdc-layer price"></div><div class="cxdc-layer part"></div><div class="cxdc-layer flow"></div><div class="cxdc-layer liq"></div><div class="cxdc-layer growth"></div><div class="cxdc-off" style="display:${info.visual.growthPresent ? "none" : "block"}">⊘</div><div class="cxdc-risk" style="opacity:${info.visual.riskReduced ? ".55" : ".98"}"></div></div></div><div class="cxdc-info"><div class="cxdc-head"><div class="cxdc-date">${escHtml(info.date)}</div><div class="cxdc-title">${T[l].daily}</div></div><div class="cxdc-formula"><b>${T[l].formula}</b><span>${escHtml(info.formula)}</span></div><div class="cxdc-footer">${escHtml(info.footer)}</div><button class="cxdc-full" type="button" aria-expanded="false" style="display:${info.full ? "block" : "none"}">${T[l].full}</button><div class="cxdc-panel" aria-hidden="true"></div></div></div>`;
    const btn = root.querySelector(".cxdc-full"), panel = root.querySelector(".cxdc-panel");
  ...
  function rerender() { render(LAST_DATA ? derive(LAST_DATA) : fallbackInfo()); }
  window.COHESIVX_RENDER_DAILY_CYLINDER = async function (data) { LAST_DATA = await hydrateWithState(data || {}); rerender(); };
  document.addEventListener("click", function (ev) { if (ev.target && ev.target.closest && ev.target.closest("#coeziv-accessibility-panel")) { setTimeout(rerender, 180); setTimeout(rerender, 650); } }, true);
  window.addEventListener("storage", function (ev) { if (ev.key === LANG_KEY) rerender(); });
})();
