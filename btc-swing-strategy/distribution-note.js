...
s.textContent = `
  ...
  .dist-note-button:active{
    transform:scale(.98);
  }

  body.light-mode .dist-note-button{
  ...
  body.light-mode .dist-note-dialog{
    background:linear-gradient(180deg,rgba(255,255,255,.98),rgba(241,245,249,.97));
    color:#0f172a;
  }
  ...
  body.light-mode .dist-note-title{
    color:#0e7490;
  }
  ...
  body.light-mode .dist-note-body{
    color:#334155;
  }
  ...
  body.light-mode .dist-note-close{
    color:#075985;
...
return nums
.slice(0, 3)
.map(x => Number(x.replace("%", "").replace("/", "").replace(",", ".")))
...
return (
parseUsdText(by("live-price")?.textContent) ||
parseUsdText(by("price")?.textContent) ||
null
);
}
...
if (
  ...
  flow.includes("sell") ||
  delta.includes("sub prețul") ||
  delta.includes("below")
) {
  return -1;
}
...
if (
  ...
  flow.includes("buy") ||
  delta.includes("peste prețul") ||
  delta.includes("above")
) {
  return 1;
}
...
if (!price) {
  return e
    ? "Concrete 24h examples are shown when a live price is available."
    : "Exemplele concrete pe 24h apar când prețul live este disponibil.";
}
...
function closeNote() {
by(MODAL_ID)?.classList.remove("open");
}
