(function(){
  ...
  function addTacticalStyle(){
    ...
    style.textContent = ''+
      ...
      '#tactical-range-slot .tr-legend{margin-top:10px;padding-top:9px;border-top:1px solid rgba(148,163,184,.16);font-size:10.5px;line-height:1.45;color:var(--text-soft);}'+
  ...
  function confidenceLegend(confidence){
    ...
    if (c === 'medium') {
      return tx('medium = range stabil sau comprimat.', 'medium = stable or compressed range.');
    }
    if (c === 'high') {
      return tx('high = confirmare tactică puternică.', 'high = strong tactical confirmation.');
    }
    return tx('low = range-ul se mișcă; semnal prudent.', 'low = range is moving; cautious signal.');
  }

  function tacticalLegend(confidence){
    return tx(
      '<strong>Legendă:</strong> BUY = preț aproape de baza range-ului · SELL = preț aproape de vârf · WAIT = fără avantaj tactic clar.<br><strong>Încredere:</strong> '+confidenceLegend(confidence),
      '<strong>Legend:</strong> BUY = price near the range low · SELL = price near the range high · WAIT = no clear tactical edge.<br><strong>Confidence:</strong> '+confidenceLegend(confidence)
    );
  }
