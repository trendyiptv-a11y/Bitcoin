(function(){
  ...
  function replaceLeaf(from,to){
    var target = norm(from);
    ...
    Array.prototype.slice.call(roots).forEach(function(root){
      Array.prototype.slice.call(root.querySelectorAll('*')).forEach(function(el){
        ...
        if (norm(el.textContent) === target) el.textContent = to;
      });
    });
  }

  function translateFixedFragments(){
    var pairs = [
    ...
    pairs.forEach(function(p){ isEn() ? replaceLeaf(p[0],p[1]) : replaceLeaf(p[1],p[0]); });
  }

  function reverseSignalLegend(){
    var grid = document.querySelector('.cxlg-grid');
    if (!grid) return;
    ['risk','sell','attention','wait','accumulate','buy'].forEach(function(k){
      var item = grid.querySelector('.cxlg-item.' + k);
  ...
  function applyGuideText(){
    ...
    Array.prototype.slice.call(document.querySelectorAll('a')).forEach(function(a){
      ...
      if (href.indexOf('ghid-mecanism.html') !== -1) {
        a.textContent = tx('Ghid mecanism','Mechanism guide');
        a.setAttribute('aria-label', tx('Deschide ghidul mecanismului','Open mechanism guide'));
      } else if (href.indexOf('mecanism.html') !== -1) {
        a.textContent = tx('← Înapoi la mecanism','← Back to mechanism');
        a.setAttribute('aria-label', tx('Înapoi la mecanism','Back to mechanism'));
      }
    });
    translateFixedFragments();
  ...
  function addLanguageFastListener(){
    function hit(ev){
      ...
      if (text === 'RO' || text === 'EN' || text.indexOf('RO/EN') !== -1 || text.indexOf('A+') !== -1 || text.indexOf('A-') !== -1) scheduleLanguageRefresh();
    }
    document.addEventListener('click', hit, true);
    document.addEventListener('touchend', hit, true);
    window.addEventListener('storage', function(ev){ if (ev && ev.key === KEY) scheduleLanguageRefresh(); });
  }
  ...
  function addTacticalStyle(){
    ...
    style.textContent = ''+
      ...
      '#tactical-range-slot .tr-meta{display:none!important;}'+
  ...
  function findStandardRegimeCard(){
    ...
    for (var i=0;i<cards.length;i++){
      ...
      if (cards[i].getAttribute('data-cohesivx-tactical-host') === '1' || text.indexOf('REGIMURI STANDARD DE PIAȚĂ') !== -1 || text.indexOf('REGIMURI STANDARD DE PIATA') !== -1) return cards[i];
    }
    return null;
  }
  ...
  function ensureTacticalSlot(){
    ...
    if (card.getAttribute('data-cohesivx-tactical-host') !== '1') {
      ...
      card.innerHTML = '<div id="tactical-range-slot"><div class="tr-title"></div><div class="tr-sub"></div><div class="tr-box"><div class="tr-row"><div class="tr-label"></div><div class="tr-signal wait">WAIT</div></div><div class="tr-message"></div><div class="tr-meta"></div><div class="tr-legend"></div></div></div>';
    }
  ...
  function renderTactical(data){
    ...
    if (signal) { signal.className = 'tr-signal ' + css; signal.textContent = sig; }
    if (message) message.textContent = isEn() ? (data.message_en || 'Wait: tactical range is updating.') : (data.message_ro || 'Așteaptă: Tactical Range se actualizează.');
  ...
  function fetchTactical(){
    ...
    fetch('./tactical_range.json?t=' + Date.now(), {cache:'no-store'})
      ...
      .then(renderTactical)
      .catch(function(){ renderTactical({tactical_signal:'WAIT',confidence:'low',message_ro:'Așteaptă: fișierul tactical_range.json nu este disponibil momentan.',message_en:'Wait: tactical_range.json is not available right now.'}); });
  ...
  function start(){
    ...
    setTimeout(translateFixedFragments, 350);
