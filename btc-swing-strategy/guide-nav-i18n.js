(function(){
  ...
  function addTacticalStyle(){
    ...
    style.textContent = ''+
      ...
      '#tactical-range-slot .tr-legend{margin-top:10px;padding-top:9px;border-top:1px solid rgba(148,163,184,.16);font-size:10.5px;line-height:1.38;color:var(--text-soft);}'+
      '#tactical-range-slot .tr-legend strong{color:var(--text-main);font-weight:750;}'+
      '#tactical-range-slot .tr-legend-row{display:block;margin-top:3px;}'+
      'body.light-mode #tactical-range-slot .tr-box{background:rgba(224,242,254,.70);border-color:rgba(14,165,233,.28);}'+
      '@media(max-width:430px){#tactical-range-slot .tr-row{align-items:flex-start;flex-direction:column;}#tactical-range-slot .tr-signal{min-width:96px;}#tactical-range-slot .tr-box{padding:12px 12px 52px 12px;}#tactical-range-slot .tr-legend{font-size:10px;line-height:1.35;padding-right:72px;}}';
  ...
  function confidenceLegend(confidence){
    ...
    if (c === 'medium') {
      return tx('medium = range stabil / comprimat.', 'medium = stable / compressed range.');
    }
    if (c === 'high') {
      return tx('high = confirmare tactică puternică.', 'high = strong tactical confirmation.');
    }
  ...
  function tacticalLegend(confidence){
    return tx(
      ...
      '<span class="tr-legend-row">BUY = aproape de baza range-ului.</span>'+
      '<span class="tr-legend-row">SELL = aproape de vârful range-ului.</span>'+
      '<span class="tr-legend-row">WAIT = fără avantaj tactic clar.</span>'+
      '<span class="tr-legend-row"><strong>Încredere:</strong> '+confidenceLegend(confidence)+'</span>',
      '<strong>Legend</strong>'+
      '<span class="tr-legend-row">BUY = near the range low.</span>'+
      '<span class="tr-legend-row">SELL = near the range high.</span>'+
      '<span class="tr-legend-row">WAIT = no clear tactical edge.</span>'+
      '<span class="tr-legend-row"><strong>Confidence:</strong> '+confidenceLegend(confidence)+'</span>'
    );
  }
